...
export function computeLiveSignals(prices, bars = {}) {
  ...
  for (const [ticker, p] of Object.entries(prices)) {
    ...
    if (st) sig.st = st;
    // sinceIndex: mevcut overzone durumunun KURULDUĞU bar — Tarama'nın "son N
    // gün" tazelik penceresi zaman damgasını (woAt) buradan türetir.
    const { cross, overzone, sinceIndex } = wavetrendSignalsFull(s.highs, s.lows, s.closes);
